...
namespace Dataformatter.Data_accessing.Filters
{
    ...
    {
        private ElectionEntity _originalElectionResultForParty;
        private PartyClassificationEntity[] _allPartyClassificationsInCountry; 
        ...
        public ElectionEntity GetElectionResultWithAbreviationAndClassification()
        {
            ...
            if (PartyClassificationAndElectionsMerger.PoliticalFamilyPerPartyInCurrentCountry.ContainsKey(_originalElectionResultForParty.PartyName))
            {
                Console.WriteLine("got " + PartyClassificationAndElectionsMerger.PoliticalFamilyPerPartyInCurrentCountry[_originalElectionResultForParty.PartyName].Abbreviation + " for " + _originalElectionResultForParty.PartyName);

                _originalElectionResultForParty.PartyAbbreviation = PartyClassificationAndElectionsMerger.PoliticalFamilyPerPartyInCurrentCountry[_originalElectionResultForParty.PartyName].Abbreviation;
            }
            else //if the above is not the case, the abrevviation is completely unknown and the user has to input it.
            {
                ...
                PartyClassificationAndElectionsMerger.PoliticalFamilyPerPartyInCurrentCountry.Add(_originalElectionResultForParty.PartyName, new PartyFamilyEntity {Abbreviation = abbreviationLine, CountryCode = _originalElectionResultForParty.CountryCode, PartyName = _originalElectionResultForParty.PartyName});
        ...
        private string AbbreviatePartyName(string partyName)
        {
