...
namespace Dataformatter.Data_accessing.Filters
{
    ...
    */
    public class PartyClassificationAndElectionsMerger
    {
        ...
        private readonly PartyFamiliesRepository _politicalFamiliesRepository = new PartyFamiliesRepository();
        ...
        public PartyClassificationAndElectionsMerger()
        {
            ...
            _countriesWithClassifiedParties = new HashSet<string>(_partyclassificationRepository.GetCountryNames());
        }
        ...
        public void Merge()
        {
            foreach (var currentCountry in _allCountriesWithElections)
            {
                ...
                if (_currentCountryCode == "NLD" && _countriesWithClassifiedParties.Contains(_currentCountryCode))
                {
                    Console.WriteLine("-------------------------------");
                    Console.WriteLine("now doing " + _currentCountryCode);
                    Console.WriteLine("-------------------------------");

                    PoliticalFamilyPerPartyInCurrentCountry = _politicalFamiliesRepository.GetDictionaryByCountry(_currentCountryCode);  //todo; DIS shit is empty. Why?
        ...
        private ElectionEntity ManuallyClassify(ElectionEntity electionResultForParty)
        {
