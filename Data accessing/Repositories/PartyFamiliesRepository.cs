...
namespace Dataformatter.Data_accessing.Repositories
{
    public class PartyFamiliesRepository : IRepository<PartyFamilyEntity>
    {
        ...
        public Dictionary<string, PartyFamilyEntity> GetDictionaryByCountry(string countryName)
        {
            if (AllFamiliesByCountry.ContainsKey(countryName))
            {
                var allClassificationsInGivenCountry = AllFamiliesByCountry[countryName];
                return allClassificationsInGivenCountry.ToDictionary(classificationEntity => classificationEntity.PartyName);
            }
